...
The language now includes indicator, signal-helper, event-memory, and early TA-Lib-style builtins such as `crossover`, `highest`, `barssince`, `valuewhen`, `ma`, `macd`, `wma`, `avgdev`, `stddev`, `linearreg`, `beta`, `correl`, and `minmax` in addition to the core OHLCV series model.
