...
pub(super) fn validate(order: &OrderDecl) -> Result<(), String> {
    ...
    {
        return Err("Hyperliquid spot trigger orders only support trigger_ref.mark".to_string());
    }
    if matches!(order.tif, Some(TimeInForce::Fok | TimeInForce::Gtd)) {
        return Err(
            "Hyperliquid spot only supports tif.gtc and tif.ioc in this backtester".to_string(),
        );
    }
    if order.post_only {
        if !matches!(
            ...
            return Err(
                "Hyperliquid spot post_only is only supported for limit-family orders".to_string(),
            );
        }
        if order.tif != Some(TimeInForce::Gtc) {
            return Err("Hyperliquid spot post_only requires tif.gtc".to_string());
        }
    }
    Ok(())
}
