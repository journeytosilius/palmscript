...
- guarda los candidatos utiles con `--preset-out best.json` para reejecutarlos con `run backtest` o `run walk-forward`, y usa `--preset-trial-id <N>` mas `--set name=value` para replay y mutacion exactos del candidato
