...
- `examples/strategies/adaptive_trend_backtest.palm`: adaptive multi-timeframe long-only backtest strategy that stays flat in bearish higher-timeframe regimes, uses breakout `market()` entries, a slower 4h trend-rollover `exit long`, a fixed `target long`, and ATR-based `protect long` chandelier exits
